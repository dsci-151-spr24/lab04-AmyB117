...
2.  What are the dimensions of the La Quinta's dataset?
  ...
  ***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for one La Quintas Inn restaurant. The variables are: `r names(laquinta)`.***
...
3.  Take a look at the websites that the data come from (linked above).
  ...
  ***La Quinta hotels are mostly located in the Americas but also have locations in other parts of the world. Denny's are located in the US.***
...
We know exactly which states are in the US, and we have this information in the `states` dataframe we loaded.
  ...
  ***We can use the State varaible to determine whether a location is in the US. Specicially, by comparing the state listed to a database of states in the US.***
...
```
  ***CA has the most and DE has the least. This is not that suprising, as California is a very popular state to travel to.***
...
```
  ***District of Columbia has the most per thousand square mile for Denny's and Rhode Island has the most per thousand square mile for Laquinta.***
...
```
  ...
  ***The joke doesn't seem to match with the data given.***
...
```
  ...
  ***For Texas, however, the joke does seem to match with our data.***
